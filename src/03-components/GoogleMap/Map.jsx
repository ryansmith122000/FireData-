...
const Map = ({ incidentData }) => {
  ...
  // ------------------------------------------------------------ If the map won't load, returns the error div -------------------------------------------------- //
  if (!isLoaded) return <div>Loading...</div>;

  /* * Explanation of the return below
  
  1. We are returning the Google Map component. I wrapped this inside of a div called "wrapper" in case of needing to display information outside of the map.
    a. If time permits, I will style this and add incident data outside of the map. 

  2. Inside of the map we are conditionally rendering the InfoWindow to make sure it's only displayed when a marker is selected.
            ...
            <div>
              ...
              {weatherData[selectedMarker.description.event_id] && (
                <div>
                  <h4>Weather Data</h4>
                  <p> Temperature:
                    {weatherData[selectedMarker.description.event_id].tavg ? ` ${formatTemp(weatherData[selectedMarker.description.event_id].tavg)}` : 'No Data'}   </p>
                  ...
                  <p> Pressure:
                    {weatherData[selectedMarker.description.event_id].pres ? ` ${weatherData[selectedMarker.description.event_id].pres} (Hg)` : 'No Data'}
                  </p>
                </div>
              )}
            </div>
